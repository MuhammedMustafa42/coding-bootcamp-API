...
exports.CreateBootcamp = async (req, res, next) => {
  try {
    ...
    req.body.user = req.user.id; // We can access user.id because this is the logged in user

    // Check for published bootcamp
    const publishedBootcamp = await Bootcamp.findOne({ user: req.user.id });

    // If user is not and admin, they can only add one bootcamp
    ...
    if (!bootcamp) {
      return next(
        ...
        if (bootcamp.user.toString() !== req.user.id && req.user.role !== "admin") {
          return next(
            new ErrorResponse(
              `User ${req.params.id} is not authorized to upload to this bootcamp`,
              401
            )
          );
        }
